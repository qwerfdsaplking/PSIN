...
class RGATConv(MessagePassing):
    ...
    def __init__(self, in_channels: int,
        ...
        self.att0 = Parameter(torch.Tensor(1, heads, out_channels))
        self.att1 = Parameter(torch.Tensor(1, heads, out_channels))
    ...
    def message(self, x_j: Tensor, x_i: Tensor, edge_type: Tensor,
        ...
        alpha = torch.zeros((x.shape[0],x.shape[1]),device=x.device)

        #print(self.att0.shape,x.shape,edge_type.shape)

        alpha[edge_type == 0] = (x[edge_type == 0] * self.att0).sum(dim=-1)
        alpha[edge_type == 1] = (x[edge_type == 1] * self.att1).sum(dim=-1)
        alpha[edge_type == 2] = (x[edge_type == 2] * (self.att0+self.att1)).sum(dim=-1)
