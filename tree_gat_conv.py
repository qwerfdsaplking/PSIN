...
class TreeGATConv(MessagePassing):
    ...
    def __init__(self, in_channels: int,
                 ...
                 share_weights: bool = False,max_depth=30,
                 **kwargs):
        super(TreeGATConv, self).__init__(node_dim=0, **kwargs)
        ...
        #self.depth_emb = Parameter(torch.Tensor(max_depth,heads,out_channels))
        self.depth_emb = nn.Embedding(max_depth,heads*out_channels)
    ...
    def forward(self, x: Union[Tensor, PairTensor], edge_index: Adj,edge_type:Tensor,
    ...
    def message(self, x_j: Tensor, x_i: Tensor, edge_type:Tensor,
        ...
        print('000',x_i.shape,self.depth_emb(edge_type).reshape(-1,self.heads,self.out_channels).shape)
        x = x_i + x_j + self.depth_emb(edge_type).reshape(-1,self.heads,self.out_channels)
